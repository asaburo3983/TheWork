...
public:
	...
	*/
	...
	void SetUpContext(VertexShader* m_VertexShader=nullptr, PixelShader* m_PixelShader=nullptr);
...
public:
	ID3D11Device* m_Device=0;												//!< @brief DirectX11のDeviceのInterface
	ID3D11DeviceContext* m_Context=0;										//!< @brief Context
	IDXGISwapChain* m_SwapChain=0;										//!< @brief SwapChainのInterface
	ID3D11RenderTargetView* m_RenderTargetView=0;							//!< @brief RenderTargetViewのInterface
	ID3D11Texture2D* m_DepthStencilTexture=0;								//!< @brief ID3D11DepthStencilViewを生成するためのテクスチャ
	ID3D11DepthStencilView* m_DepthStencilView=0;							//!< @brief DepthStencilViewのInterface
	ID3D11SamplerState* m_SamplerState=0;									//!< @brief Textureサンプラー
	ID3D11Buffer* m_ConstantBuffer=0;										//!< @brief 定数バッファ
	ConstantBuffer m_ConstantBufferData;								//!< @brief ConstantBufferデータ

	ID3D11BlendState* g_pBlendState=0;			// ブレンド・ステート・オブジェクト
	ID3D11DepthStencilState* g_pDepthStencilState=0;
	ID3D11RasterizerState* g_pRasterizerState=0;
