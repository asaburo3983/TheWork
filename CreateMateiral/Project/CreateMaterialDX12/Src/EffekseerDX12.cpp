...
void efk::Effect::Load(const char16_t* ufilepath) {
...
void efk::Effect::Play(Vector3 _pos,Vector3 _rota,Vector3 _scale,int _endTime) {
	auto lowApp = LowApplication::GetInstance();

	// フレームの開始時に呼ぶ
	sfMemoryPoolEfk->NewFrame();

	camera3D::Camera* cam = camera3D::Camera::GetInstance();
...
void efk::Effect::SetTime(int _time) {
	time = _time;
}
