...
void lowApp::LowApplication::EnableDebugLayer() {
	if (!debugMode)return;
...
void lowApp::LowApplication::Init(const char* _winName, int _windowSizeX, int _windowSizeY, bool _fullscreen) {
...
void lowApp::LowApplication::InitWindow(const char* winName, int _windowSizeX, int _windowSizeY, bool _fullscreen) {
...
HRESULT lowApp::LowApplication::InitDevice() {
	UINT flagsDXGI = 0;
...
HRESULT lowApp::LowApplication::InitCommand() {
...
HRESULT lowApp::LowApplication::InitSwapChain() {
...
HRESULT lowApp::LowApplication::InitRenderTarget() {
...
HRESULT lowApp::LowApplication::InitDepth() {
...
HRESULT lowApp::LowApplication::InitFence() {
	fence = nullptr;
	fenceValue = 0;
	return  device->CreateFence(fenceValue, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&fence));
}

void lowApp::LowApplication::InitDefaultPipelineStateDesc() {
...
void lowApp::LowApplication::SetSampleMaskAsDefaultPipelineState() {

	pipelineStateDesc[(int)PipelineType::DEFAULT].SampleMask = D3D12_DEFAULT_SAMPLE_MASK;
}
void lowApp::LowApplication::SetBlendStateAsDefaultPipelineState() {
...
void lowApp::LowApplication::SetRasterizerStateAsDefaultPipelineState() {
	//ラスタライザー
...
void lowApp::LowApplication::SetDepthStencilStateAsDefaultPipelineState() {
	//デプスバッファ
...
void lowApp::LowApplication::SetTopologyTypeAsDefaultPipelineState() {
	//ポリゴンの変形（三角にするなど）
...
void lowApp::LowApplication::SetRenderTargetsAsDefaultPipelineState() {
	//レンダーターゲット
...
void lowApp::LowApplication::SetSampleDescAsDefaultPipelineState() {
	//サンプリング数の決定
...
void lowApp::LowApplication::SetViewportAsDefaultPipelineState() {
	viewport.resize(1);
...
void lowApp::LowApplication::ChangeResourceState(D3D12_RESOURCE_STATES _before, D3D12_RESOURCE_STATES _after) {
...
void lowApp::LowApplication::DrawStart() {
...
void lowApp::LowApplication::DrawOnRenderTarget(ID3D12DescriptorHeap* _renderTargetBufferHeap) {
...
void lowApp::LowApplication::DrawOnRenderTargetNoDepth(ID3D12DescriptorHeap* _renderTargetBufferHeap) {
	ChangeResourceState(D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
...
void lowApp::LowApplication::DrawOnScreen() {
...
void lowApp::LowApplication::DrawFinish() {
...
void lowApp::LowApplication::DrawOnDepth(ID3D12DescriptorHeap* depthBufferHeap,int texSize){
...
//終了処理
void lowApp::LowApplication::UnInit() {
