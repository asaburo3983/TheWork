...
{
	cBuffer::MaterialBuffer entry_material;
...
bool FBX::LoadTexture(FbxFileTexture* texture, std::string& keyword, string _texturePath)
{
	...
	result = lowApp::LowApplication::GetInstance()->GetDevice()->CreateCommittedResource(
		&texHeapProp,
...
{
	auto device = lowApp::LowApplication::GetInstance()->GetDevice();
...
void FBX::Draw(
	...
	//カメラ情報を渡す
	cBuffer::CameraBufferHeap* cameraBufferHeap = camera3D::Camera::GetInstance()->GetConstantBuffer();
	...
	//ディレクショナルライト情報を渡す
	cBuffer::LightBufferHeap* directionalLightBufferHeap= light::Lights::GetInstance()->GetConstantBuffer();
	...
	//深度テクスチャ情報を渡す
	light::Lights* dLight = light::Lights::GetInstance();
	...
	for (auto& mesh : meshList)
	{
		...
		//アルベド以外のテクスチャ情報を渡す
		for (int h = 0; h < (int)shader::TextureSlot::MODEL_ANOTHER; h++) {
