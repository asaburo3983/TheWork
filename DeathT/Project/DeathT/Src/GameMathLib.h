...
namespace GameMath {
	...
	Pos2 GetCirclePoint(float posX, float posY, float r, float  angle);//Yの上がマイナス　（２D）
	Pos2 GetCirclePointPosiY(float posX, float posY, float r, float  angle);//Yの上がプラス
	//位置から角度を算出
	float GetAngle(Pos2 pos0, Pos2 pos1);
	float GetAnglePosi(Pos2 pos0, Pos2 pos1);//Yが逆(下に行くほど値が減る)3Dで多様すると思う
